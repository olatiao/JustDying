...
public class AttributeHelper {
     ...
     * @param player 玩家
     */
    public static void updateAllAttributes(ServerPlayerEntity player) {
        if (player == null) {
            return;
        }
        ...
        try {
            ...
            AttributeManager.getAllAttributes().forEach(attribute -> {
                ...
                int currentValue = getAttributeValue(player, id);
                int minValue = attribute.getMinValue();
                int maxValue = attribute.getMaxValue();
                
                // 确保属性值在有效范围内
                if (currentValue < minValue) {
                    setAttributeValue(player, id, minValue);
                } else if (currentValue > maxValue) {
                    setAttributeValue(player, id, maxValue);
                }
                
                // 如果是调试模式，输出日志
                if (JustDying.getConfig().debug) {
                    JustDying.LOGGER.debug("更新玩家 {} 的属性 {} = {}", 
                            player.getName().getString(), id, currentValue);
                }
            });
        } catch (Exception e) {
            JustDying.LOGGER.error("更新玩家属性时出错: {}", e.getMessage());
        }
    }
