...
public class AttributeCommands {
                                ...
                                .then(CommandManager.argument(ARG_ATTRIBUTE, StringArgumentType.word())
                                        .suggests((context, builder) -> {
                                            AttributeManager.getAllAttributes()
                                                    .forEach(attr -> builder.suggest(attr.getId().getPath()));
                                            return builder.buildFuture();
                                        })
                                        .executes(context -> getAttributeCommand(context,
    ...
    private static void sendInfoMessage(CommandContext<ServerCommandSource> context, String format, Object... args) {
        ...
        context.getSource().sendFeedback(() -> Text.translatable(format, args).formatted(Formatting.AQUA), false);
